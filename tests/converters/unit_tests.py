...
@patch("tflite_utils.keras_to_tflite")
def test_convert_keras_model_to_tflite(mock_convert, converter, mock_keras_model):
    mock_convert.return_value = "model.tflite"
    result = converter.convert(model=mock_keras_model, output_path="model.tflite")
    mock_convert.assert_called_once()
    assert result == "model.tflite"

@patch("tflite_utils.onnx_to_tflite")
